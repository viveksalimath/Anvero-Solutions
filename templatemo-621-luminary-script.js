...
function setPricing() {
  ...
  if (mLabel) mLabel.classList.toggle('active', !annual);
  if (aLabel) aLabel.classList.toggle('active', annual);
  if (saveBadge) saveBadge.classList.toggle('show', annual);
...
const industryPanes = document.querySelectorAll('.industry-pane');

industryBtns.forEach(btn => {
  btn.addEventListener('click', () => {
    const targetId = btn.dataset.industry;
    industryBtns.forEach(b => b.classList.toggle('active', b === btn));
    industryPanes.forEach(pane => {
      pane.classList.toggle('active', pane.id === `industry-${targetId}`);
    });
  });
});

// ── 5-Stage BPO Framework Tabs ──
const frameworkStepBtns = document.querySelectorAll('.framework-step-btn');
const frameworkPanes = document.querySelectorAll('.framework-pane');

frameworkStepBtns.forEach(btn => {
  btn.addEventListener('click', () => {
    const stepNum = btn.dataset.step;
    frameworkStepBtns.forEach(b => b.classList.toggle('active', b === btn));
    frameworkPanes.forEach(pane => {
      pane.classList.toggle('active', pane.id === `framework-step-${stepNum}`);
    });
  });
});

// ── Modals Controller (Privacy, Terms, Service Scopes) ──
...
// ── Free Assessment Booking Form Handling ──
...
if (assessmentForm) {
  assessmentForm.addEventListener('submit', e => {
    e.preventDefault();
    const submitBtn = assessmentForm.querySelector('.form-submit-btn');
    const origText = submitBtn.textContent;
    
    submitBtn.textContent = 'Analyzing & Confirming...';
    submitBtn.disabled = true;

    setTimeout(() => {
      const name = assessmentForm.querySelector('[name="name"]').value || 'there';
      const company = assessmentForm.querySelector('[name="company"]').value || 'your company';
      const time = assessmentForm.querySelector('[name="preferredTime"]').value || 'Morning (9am-12pm)';
      
      if (formStatusMsg) {
        formStatusMsg.innerHTML = `
          <strong>✓ Assessment Request Received!</strong><br>
          Thank you, <strong>${name}</strong>. Our senior BPO operations architects are reviewing the call center requirements for <strong>${company}</strong> and will connect during your preferred window (<em>${time}</em>). A custom diagnostic blueprint has been queued.
        `;
      ...
      submitBtn.textContent = 'Assessment Request Confirmed ✓';
